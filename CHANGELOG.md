...
- **Path allowlist is now provenance-gated (blind spot closed, not just documented).** `RedactPath` takes the path's kernel provenance (`PathResolved`/`PathClaimed`, the existing `path_source`/D7 signal, threaded from the enricher). The content-addressed allowlist (hash-shaped segments under `objects`/`commits`) applies **only to resolved paths** — where the kernel dentry walk vouches the file exists — so a real git/nix content hash is still spared (no precision loss), but a hex segment an attacker crafts into a `claimed` path is redacted. The zero value is `PathClaimed` (fail-safe). This is the principled resolution of the precision/leak tension: benign hashes and crafted secrets are indistinguishable by *shape* but perfectly distinguishable by *provenance*. Residual narrows to a covert channel (an agent that creates a real file named after a hex-encoded secret) and the UUID-shape case (below the entropy bar, unredactable without shredding all UUIDs) — both in `LIMITS.md §4`.
...
- **Docs honesty (P10):** `docs/REDACTION.md` §4 and `LIMITS.md` §4 corrected — the marker is a correlation *hint*, not a commitment (a salt-holder can confirm a guessed low-entropy value); the residual is stated plainly (context-free short values, and the two narrow path residuals above). The fuzzer gained a real leak assertion (a sentinel secret must redact under any fuzz-chosen context), not just no-panic/idempotency.
