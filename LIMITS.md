...
- **Path-shaped allowlist blind spot.** The entropy pass allowlists content-addressed path segments (a hash-length hex file under an `objects`/`commits` directory, or a valid RFC-4122 v4 UUID) so it does not shred the very paths a file event exists to record. A secret an attacker *crafts* to look like one of those, placed in a `path_source=claimed` field, escapes the entropy pass. Structural provider patterns (AWS/GCP/… keys) still run on those segments, so a real provider-shaped key embedded there is still caught.
