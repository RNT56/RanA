...
| **Resolved paths are mount-relative (watchlist prefixes on non-root mounts)** | The in-kernel resolve walk follows `d_parent` within one mount and does not cross mount points, so a file on a separate mount resolves as its **mount-relative** path — on a tmpfs `/tmp`, `/tmp/secret` resolves as `/secret`. An absolute-path *prefix* watchlist entry for a file on such a mount therefore never matches. Found by the multi-kernel harness's D9 leg (the watchlisted read produced no event on any kernel until the test file moved to the root mount). | Files that exist at session start are **also pinned by `(dev,inode)`**, which is mount-agnostic — pre-existing credential files (the primary D9 target: `~/.ssh/*`, cloud credential stores) are caught via inode identity even where prefix matching misses. Affected: files *created mid-session* on a non-root mount and matched purely by prefix. Recorded paths for ordinary `fs.*` events on those mounts are likewise mount-relative (still `path_source=resolved` — a true, bounded walk of that mount). | Fix planned: cross mounts via `mount->mnt_parent`/`mnt_mountpoint` (the structs are already declared for it). Until then, register watchlist prefixes mount-relatively where the layout demands it, or rely on inode pinning. |
