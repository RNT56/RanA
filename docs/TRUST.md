...
5. **Gap honesty** — confirm every claimed `gap` is internally consistent, that row continuity matches segment bounds, and that each segment header's `gap_summary` **equals the tally of `gap` events actually present in that segment** (decoded from the segment's own merkle-protected bytes, never the mutable mirror columns). This last cross-check matters most on the *unattested tail*: a signed segment's `gap_summary` is already bound by the checkpoint signature, but the not-yet-signed tail could otherwise have a gap silently suppressed by a raw-storage attacker who recomputes a self-consistent `seg_hash` — cross-checking against the events makes that inconsistency a `gap_dishonest` finding (P5).
